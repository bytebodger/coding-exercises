/*
   Range Sum Query 2D - Immutable
   Tags: Array, Design, Matrix, Prefix Sum
   Given a 2D matrix matrix, handle multiple queries of the following type:

   Calculate the sum of the elements of matrix inside the rectangle defined by its upper left corner
   (row1, col1) and lower right corner (row2, col2).
   Implement the NumMatrix class:

   NumMatrix(int[][] matrix) Initializes the object with the integer matrix matrix.
   int sumRegion(int row1, int col1, int row2, int col2) Returns the sum of the elements of matrix inside
   the rectangle defined by its upper left corner (row1, col1) and lower right corner (row2, col2).
   You must design an algorithm where sumRegion works on O(1) time complexity.

   Example 1:
   Input
   ["NumMatrix", "sumRegion", "sumRegion", "sumRegion"]
   [[[[3, 0, 1, 4, 2], [5, 6, 3, 2, 1], [1, 2, 0, 1, 5], [4, 1, 0, 1, 7], [1, 0, 3, 0, 5]]], [2, 1, 4, 3], [1, 1, 2, 2], [1, 2, 2, 4]]
   Output
   [null, 8, 11, 12]
   Explanation
   NumMatrix numMatrix = new NumMatrix([[3, 0, 1, 4, 2], [5, 6, 3, 2, 1], [1, 2, 0, 1, 5], [4, 1, 0, 1, 7], [1, 0, 3, 0, 5]]);
   numMatrix.sumRegion(2, 1, 4, 3); // return 8 (i.e sum of the red rectangle)
   numMatrix.sumRegion(1, 1, 2, 2); // return 11 (i.e sum of the green rectangle)
   numMatrix.sumRegion(1, 2, 2, 4); // return 12 (i.e sum of the blue rectangle)

   Constraints:
   m == matrix.length
   n == matrix[i].length
   1 <= m, n <= 200
   -10^4 <= matrix[i][j] <= 10^4
   0 <= row1 <= row2 < m
   0 <= col1 <= col2 < n
   At most 104 calls will be made to sumRegion.

   Complexity:
   Time: O(n^2) for the constructor, O(1) for sumRegion
   Space: O(n^2)
 */

export class NumMatrix {
   private readonly memo: number[][];

   constructor (matrix: number[][]) {
      const height: number = matrix.length;
      const width: number = matrix[0].length;
      this.memo = [];
      let leftSum: number;
      for (let row = 0; row < height; row++) {
         this.memo.push([]);
         leftSum = 0;
         for (let column = 0; column < width; column++) {
            leftSum += matrix[row][column];
            if (row === 0)
               this.memo[row].push(leftSum);
            else
               this.memo[row].push(leftSum + this.memo[row - 1][column]);
         }
      }
   }

   sumRegion (row1: number, col1: number, row2: number, col2: number): number {
      let sum: number = this.memo[row2][col2];
      if (row1 > 0)
         sum -= this.memo[row1 - 1][col2];
      if (col1 > 0)
         sum -= this.memo[row2][col1 - 1];
      if (row1 > 0 && col1 > 0)
         sum += this.memo[row1 - 1][col1 - 1];
      return sum;
   }
}