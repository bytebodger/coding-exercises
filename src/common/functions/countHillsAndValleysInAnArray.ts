/*
   ...
   Note that for an index to be part of a hill or valley, it must have a non-equal neighbor on both the left
...
export const countHillValley = (values: number[]): number => {
   let result = 0;
   for (let i = 1; i < values.length; i++) {
      ...
      if (value === previousValue)
         continue;
      let rightSide = i + 1;
      let rightValue = values[rightSide];
      while (value === rightValue) {
         rightSide++;
         rightValue = values[rightSide];
      }
      if ((previousValue < value && value > rightValue) || (previousValue > value && value < rightValue))
         result++;
   }
   return result;
}