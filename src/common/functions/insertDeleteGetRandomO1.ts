...
export class RandomizedSet {
   ...
   insert = (value: number): boolean => {
      if (this.map.has(value))
         ...
         this.map.set(value, this.values.length);
   ...
   remove = (value: number): boolean => {
      if (this.map.has(value)) {
         if (this.values[this.values.length - 1] !== value) {
            const movedValue = this.values[this.values.length - 1]
            const removedIndex = this.map.get(value);
            this.values[removedIndex] = movedValue;
   ...
   getRandom = (): number => this.values[Math.floor(Math.random() * this.values.length)];
}