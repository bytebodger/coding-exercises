/*
   ...
   when a function call ends, its ID is popped off the stack. The function whose ID is at the top of the stack
   is the current function being executed. Each time a function starts or ends, we write a log with the ID, whether
   ...
   started at the beginning of timestamp 3, and "1:end:2" means a function call with function ID 1 ended at the
...
export const exclusiveTime = (totalFunctions: number, logs: string[]): number[] => {
   const stack = [];
   const results = new Array(totalFunctions).fill(0);
   let previous = 0;
   for (const log of logs) {
      ...
      if (flag === 'start') {
         ...
         if (stack.length > 0)
            results[stack[stack.length - 1]] += parseInt(time) - previous;
         previous = parseInt(time);
      ...
      if (flag === 'end') {
         ...
         if (lastStack === undefined)
            continue;
         results[lastStack] += parseInt(time) - previous + 1;
         previous = parseInt(time) + 1;
      }
   }
   return results;
}