/*
   ...
   Time complexity: O(n^2)
      where n is the length of accounts
   Space complexity: O(2 * n^2)
 */

export const accountsMerge = (accounts: string[][]): string[][] => {
   // for a given email
   //    add it to visited[] and accessedEmails[]
   //    if it exists in the graph, seek() for child email
   const seek = (email: string) => {
      if (visited.has(email))
         return;
      visited.add(email);
      accessedEmails.push(email);
      const emailGraph = graph.get(email);
      if (emailGraph !== undefined)
         emailGraph.forEach(seek);
   }

   const graph = new Map<string, Set<string>>();
   const owner = new Map<string, string>();
   // build graph and owner
   accounts.forEach(([name, ...emails]) => {
      ...
      emails.forEach(email => {
         ...
         const emailGraph = graph.get(email);
         const firstEmailGraph = graph.get(firstEmail);
         if (emailGraph !== undefined)
            emailGraph.add(emails[0]);
         if (firstEmailGraph !== undefined)
            firstEmailGraph.add(email);
      })
      owner.set(firstEmail, name);
   })
   const result: string[][] = [];
   const visited = new Set<string>();
   let accessedEmails: string[];
   for (const email of graph.keys()) {
      accessedEmails = [];
      // populate visited[] and accessed[] for this email
      seek(email);
      // if it's been accessed, the add the owner => accessedEmails[] values to the result
      if (accessedEmails.length > 0) {
         const emailOwner = owner.get(email);
         if (emailOwner !== undefined)
            result.push([emailOwner, ...accessedEmails.sort()]);
      }
   }
   return result;
}