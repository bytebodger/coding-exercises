/*
   ...
   which each node has no more than N children. There is no restriction on how your serialization/deserialization
   algorithm should work. You just need to ensure that an N-ary tree can be serialized to a string and
...
export class Codec {
   serialize = (root: _Node): string => JSON.stringify(root);

   deserialize = (data: string): _Node => JSON.parse(data) as _Node;
}